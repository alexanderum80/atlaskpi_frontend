...
import { IMilestone, ITargetNew, ITargetFormFields } from '../shared/models/targets.model';
...
export class FormTargetsComponent implements OnInit {
    ...
    @ViewChild('listTarget')
    listTarget: ListTargetsComponent;
    @ViewChild('basic')
    basicForm: BasicTargetsComponent;
    @ViewChild('milestoneComponent')
    milestoneComponent: MilestoneComponent;
    @ViewChild('related')
    relatedComponent: RelatedUsersComponent;
    ...
    constructor(
        public vm: FormTargetsViewModel,
    ...
    onDelete(event) {
        this._apolloService
            .networkQuery<ITargetNew>(findTargetByName, {
                name: this.vm.name,
            })
            .then(res => {
    ...
    deleteOld(id) {
        this._apolloService
            .mutation<ITargetNew>(removeTarget, {
                id: id,
    ...
    private fields() {
        ...
        const fields: ITargetFormFields = {
            name: that.vm.name,
            compareTo: that.vm.compareTo,
            kpi: that.vm.kpi,
            recurrent: that.vm.recurrent,
            source: {
                identifier: that.chart._id,
                type: 'chart'
            },
            ...
            notificationConfig: {
                ...
                }) as any[] // HACK: this whole module its bad I just want to get it out
            }
        ...
        if (that.chart.groupings.length > 0 || !that.chart.frequency) {
            if (categories.length === 0) {
                ...
                series.forEach(serie => {
                    const filterSerie = filter(that.targets, {
                        name: serie.name,
                    });
    ...
    private _isStacked(groupings: string[], chartData: any) {
        return (
            (chartData.chartDefinition.chart.type === 'column' &&
    ...
    private _refreshTargets(id?: string) {
        ...
        this._apolloService.networkQuery<ITargetNew[]>(targesQuery, source).then(d => {
            ...
            if (that.targets.length === 0) {
                ...
                that.item = {
                    id: that.vm.fg.controls._id.value,
                    selected: true,
                };
    ...
    private actionAdd() {
        ...
        this._apolloService
            .mutation<ITargetNew>(addTargetsMutation, {
                TargetNewInput: input,
            })
            .then(res => {
                that.getId();
            })
    ...
    private getId() {
        const that = this;
        this._apolloService
            .networkQuery<ITargetNew>(trargetByName, {
                name: that.vm.name,
            })
            .then(res => {
                ...
                if (that.milestoneComponent.tempTarget.length > 0) {
                    that.milestoneComponent.tempTarget.forEach(element => {
                        element.target = id;
                        this._apolloService
                            .mutation<IMilestone>(addMilestone, {
                                input: element,
                            })
                            .then(data => {
                                ...
                                that._milestoneService.userMilestoneNotification(resp, {
                                    task: task,
                                    dueDate: dueDate,
                                });
                            })
    ...
    private actionEdit() {
        ...
        this._apolloService
            .mutation<ITargetNew>(editTargetsMutation, {
                id: this.vm._id,
                TargetNewInput: input,
            })
            .then(res => {
    ...
    private subscriptionName() {
        ...
        that._subscription.push(
            that.vm.fg.controls['name'].valueChanges.subscribe(value => {
